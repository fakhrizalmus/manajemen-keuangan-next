...
import { getPemasukan, kategoriPemasukan, putPemasukan } from "./actions"
...
import { DataPemasukan } from "./columns"
...
export default function EditModal({ id, onClose, onSuccess }: EditModalProps) {
    ...
    React.useEffect(() => {
        if (id !== null) {
            kategoriPemasukan().then((res) => {
                setKategoriList(res.data);
            })
            getPemasukan({ id }).then((res) => {
                const data: DataPemasukan = res.data.rows[0]
                ...
                setForm({
                    ...
                    tanggal,
                    kategori_pemasukan_id: data.KategoriPemasukan?.id || 0,
                        ...
                        <div className="grid gap-3">
                            ...
                            <Popover open={open} onOpenChange={setOpen}>
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                    {form.kategori_pemasukan_id
                                        ? kategoriList.find((item) => item.id === form.kategori_pemasukan_id)?.nama_kategori
                                        : "Pilih kategori..."}
                                ...
                                <PopoverContent className="w-full p-0">
                                    ...
                                    <CommandList>
                                        ...
                                        {kategoriList.map((item) => (
                                            ...
                                            onSelect={() => {
                                                setForm({ ...form, kategori_pemasukan_id: item.id });
                                                setOpen(false);
                                            }}
                                            >
                                            {item.nama_kategori}
